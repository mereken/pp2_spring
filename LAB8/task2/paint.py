...
def draw_rect(surface, color, start_pos, end_pos):
    pygame.draw.rect(surface, color, pygame.Rect(start_position, (end_pos[0] - start_position[0], end_pos[1] - start_position[1])))
...
while running:
    ...
    if left_button_pressed or right_button_pressed:
        if drawing_mode == "brush":
            ...
            if left_button_pressed:
                draw_rect(screen, current_color, start_position, pygame.mouse.get_pos())
        elif drawing_mode == "circle":
            if left_button_pressed:
                radius = ((pygame.mouse.get_pos()[0] - start_position[0])**2 + (pygame.mouse.get_pos()[1] - start_position[1])**2)**0.5
                draw_circle(screen, current_color, start_position, int(radius))
